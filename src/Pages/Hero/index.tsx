...
export const Hero = () => {
  ...
  useEffect(() => {
    if (firstPlaces.length > 0 && ListPerson.length > 0) {
      const timer = setTimeout(() => setIsLoading(false), 1500);
      return () => clearTimeout(timer);
    }
  }, [firstPlaces, ListPerson]);
  ...
  return (
    <>
      ...
      <section className="w-full h-full flex flex-col xl:flex-row justify-center items-start gap-8 md:px-12 2xl:px-32">
          ...
          {showAnimation.state && (
            ...
            >
              <motion.div
                ...
                style={{
                  backgroundImage: `url(${showAnimation.typePlaces === 'first' ? ImageshieldOne : showAnimation.typePlaces === 'second' ? ImageshieldTwo : ImageshieldThree})`
                }}
        ...
        {/* Pódio */}
        <div className={`w-full xl:w-[55%] min-h-[18rem] md:min-h-[44rem] bg-[#1f1e2b] rounded-3xl flex justify-center items-center gap-6 sm:gap-8 relative bg-cover bg-center py-8 sm:py-0 transition-all duration-300 ${isLoading ? 'blur-sm opacity-60' : ''}`} style={{ backgroundImage: `url(${BgList})` }}>
          {/* 2º Lugar */}
          <div className="relative z-10 flex flex-col items-center">
            <div className="w-20 h-32 sm:w-24 sm:h-40 xl:w-40 xl:h-64 bg-center bg-cover animate-floatSecound flex flex-col items-center justify-center" style={{ backgroundImage: `url(${ImageshieldTwo})` }}>
              <img src={firstPlaces[1]?.urlImage || Avatar} className="octagon w-11 h-12 sm:w-19 sm:h-20 xl:w-26 xl:h-28 mx-auto mt-2" alt="Imagem" />
          ...
          <div className="relative z-30 flex flex-col items-center">
            <div className="w-25.5 h-40 sm:w-28 sm:h-48 xl:w-48 xl:h-76 bg-center bg-cover animate-float flex flex-col items-center justify-center" style={{ backgroundImage: `url(${ImageshieldOne})` }}>
              <img src={firstPlaces[0]?.urlImage || Avatar} className="octagon w-12 h-13 sm:w-19 sm:h-20 xl:w-28 xl:h-30 mx-auto mt-2" alt="Imagem" />
          ...
          {/* 3º Lugar */}
          <div className="relative z-10 flex flex-col items-center">
            <div className="w-20 h-32 sm:w-24 sm:h-40 xl:w-40 xl:h-64 bg-center bg-cover animate-floatThree flex flex-col items-center justify-center" style={{ backgroundImage: `url(${ImageshieldThree})` }}>
              <img src={firstPlaces[2]?.urlImage || Avatar} className="octagon w-11 h-12 sm:w-19 sm:h-20 xl:w-26 xl:h-28 mx-auto mt-2" alt="Imagem" />
        ...
        {/* Lista de Pessoas */}
        <div className={`w-full xl:w-[45%] min-h-[28rem] md:min-h-[44rem] p-4 sm:p-5 xl:p-9 bg-[#1f1e2b] grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 xl:gap-7 items-center rounded-4xl bg-cover bg-center transition-all duration-300 ${isLoading ? 'blur-sm opacity-60' : ''}`} style={{ backgroundImage: `url(${BgList})` }}>
          <AnimatePresence>
            {ListPerson.sort((a, b) => Number(b.total_vendas) - Number(a.total_vendas)).map((list, index) => {
              const positionImg = index === 0 ? ImageFirstPlaces : index === 1 ? ImageSecondPlaces : index === 2 ? ImageThirdPlaces : null;
